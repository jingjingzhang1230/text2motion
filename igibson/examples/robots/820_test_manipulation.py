...
class Arguments:
    def __init__(self):
        ...
        self.n_objects = 3 # only 3 objects are spawned
    ...
    def move_hand_to_position(self, env, target_pos, body_id=-1):
        ...
        # it should work and is shoudl
            ...
            if plan and len(plan) > 0:
                ...
                planned_paths = []
                planned_paths.append(plan)

                # if planned_paths is not None and len(planned_paths) > 0:

                #     for i in range(len(planned_paths)):

                #         # if we have multiple path, we have to load the robot to the origin location when we play the path
                ...
                #         # write get_colur as function when we want fixed colors

                #         for j in range(0, len(path), 5):#sample every 5 waypoints to plot
                ...
                #             markers[marker_idx].set_position([way_point[0], way_point[1], way_point[2]])
                #             marker_idx += 1

                #             print(
                #                 "The x,y and z of path are shown: "
                #             )
                #             print(way_point[0], way_point[1], way_point[2])

                #             print("The length of path is :" + str(len(plan)))
                ...
                for i, n in enumerate(plan):
                    set_joint_positions(self.body_id, self.arm_joint_ids, n)
                    self.env.step(None)
                    logging.debug(f"Arm push step {i} / {plan_len}")
                    print(f"Arm push step {i} / {plan_len}")

                    way_point_task_space  = self.reference.get_eef_position()
                    markers.append(VisualMarker(visual_shape=p.GEOM_SPHERE, radius=0.06,rgba_color=marker_color))
                    env.simulator.import_object(markers[marker_idx])
    ...
    while True:
        ...
        with Profiler("Environment step", logger=logger):
            ...
            if "push_objects" in args.behavior and counter % 5 == 0 and not fetch.moving:
                ...
                needs_new_picture = fetch.move_hand_to_position(env, target_pos, target_obj.get_body_ids()[0])
...
# let say we only want fixed 2 items, 'mouse' and 'cup'
def setup_scene(env, fetch, object_indeces=None, n=2):
    ...
    # the original denominator was 3, change to gretaer values to let the objects spawn at more central region of the table.
        x_dist = (table_aabb_max[0] - table_aabb_min[0]) / 10
        y_dist = (table_aabb_max[1] - table_aabb_min[1]) / 10
    ...
    # 每次run 都会生成一个新的table_pos，所以必须重新获取位置
        x_d1 = [table_pos[0] - x_dist, table_pos[0] + x_dist]
        y_d1 = [table_pos[1] - y_dist, table_pos[1] + y_dist]

    counter = 1
    for obj in spawned:
            if counter == 1:
                obj.set_position_orientation([x_d1[0], y_d1[0], table_aabb_max[2] + 0.5],
                                            [0, 0, 0, 1])
                print("first obj spawned at: " + str(x_d1[0]) + str(y_d1[0]))
            elif counter == 2:
                obj.set_position_orientation([x_d1[1], y_d1[1], table_aabb_max[2] + 0.5],
                                            [0, 0, 0, 1])
                print("second obj spawned at: " + str(x_d1[1]) + str( y_d1[1]))
            counter += 1
