        ...
        with Profiler("Environment step", logger=logger):
            ...
            if "push_objects" in args.behavior and counter % 5 == 0 and not fetch.moving:
                ...
                while fetch.plan_counter < (np.array(env.config["sample_number"])):
                    ...
                    #     env.land(target_obj, reset_pos, [0, 0, 0])
                    #     fetch.prepare_for_picture()
                    #     print("The object has been reset.")
                # print(fetch.plan_counter)
                # print(str(all_plans))
                # print(np.asarray(fetch.plan).shape)
                # print(np.asarray(all_plans).shape)

    # start clustering
    ####### CLUSTERING PATHS #############
    # Function to extract statistical features from each path
                def extract_features(path):
                    ...
                    ##### Alternative feature:
                    # # Sample 5 points from the path
                    # sampled_points = np.linspace(0, len(dim_values) - 1, 5, dtype=int)
                    # sampled_values = dim_values[sampled_points]
                    # features.extend(sampled_values.tolist())
                    return features
                    # Extract features for all paths
                ...
                num_clusters = 2 #config_data['num_clusters']
